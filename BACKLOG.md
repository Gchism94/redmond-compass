...
|---|---|---|---|
| Fast-follow | **Complete owner-system mirror** | The owner's Base44/GHL main site remains authoritative. The app mirrors every public non-ranking Business field, approved events, business posts, classes, and news every six hours. All new intake and canonical edits route to the current main-site forms; app content tables are browser-read-only. | Inventory the private Base44/GHL automations and add server-side contracts for anything not exposed publicly, especially approved `BusinessPhoto` galleries. |
