...
|---|---|---|---|
| Fast-follow | **Shared main-site content source** | Businesses and news now have guarded, six-hour main-site → Supabase bridges; events sync from Google Calendar every six hours. `CONTENT-SYNC.md` records ownership, cadence, and evidence. | Inventory the private Base44/GHL automations, make the main site read published Supabase businesses/events/news, migrate remaining Base44 media, then retire both temporary bridges. |
...
| 6 | **Complete and verify directory fields** | Main-site audit on 2026-09-02: 147 approved, profile-enabled listings. 114 have authored hours (66 parse to structured weekly schedules, 46 remain accurate prose, and 2 claimed-owner schedules are preserved); 33 have no hours on the main site itself. The app labels uncertain schedules as unknown, hides unavailable actions, and never treats fallback coordinates as a real location. | The directory owner must supply or explicitly mark appointment/variable hours for the remaining 33 source records. |
